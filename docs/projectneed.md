# mono-agent 项目需求与语义不变量

这份文件是 mono-agent 的长期需求规范。它回答“系统必须保持什么”，供新会话、维护者、coding agent、评审者和 CI 使用。
...
mono-agent 必须在多轮会话、进程重启、插件换代和工作区切换后保留用户授权保存的事实。临时预算、展示窗口和缓存策略不得改写数据保留范围。
